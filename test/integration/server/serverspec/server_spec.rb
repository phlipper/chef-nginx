# Server recipe is the only one that installs the package. This means I am testing
# Everthing in one suite. The aim would be to separate the installation so that
# the tests can also be in their own files.
require 'serverspec'
...
describe 'chef-nginx::server' do
  ...
  describe 'chef-nginx::configuration' do
    ...
    describe 'mime.types' do
      let(:mime_types) { file '/etc/nginx/mime.types' }
    ...
    describe 'nginx_conf' do
      let(:nginx_conf) { file '/etc/nginx/nginx.conf' }
    ...
    describe 'config files' do
      describe 'buffers' do
        let(:buffers_conf) { file '/etc/nginx/conf.d/buffers.conf' }
        it 'is included' do
